...
function blockedHostnameMessage(hostname: string): string {
  return (
    `Hostname '${hostname}' is not allowed for this OrchestorAI instance. ` +
    `If you want to allow this hostname, please run pnpm orchestorai allowed-hostname ${hostname}`
  );
}
...
export function privateHostnameGuard(opts: {
  ...
  return (req, res, next) => {
    ...
    if (!hostname) {
      const error = "Missing Host header. If you want to allow a hostname, run pnpm orchestorai allowed-hostname <host>.";
      if (wantsJson) {
