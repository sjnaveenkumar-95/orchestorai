...
const DEFERRED_WAKE_CONTEXT_KEY = "_orchestoraiWakeContext";
...
export function heartbeatService(db: Db) {
  ...
  async function executeRun(runId: string) {
    ...
    context.orchestoraiWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    try {
      ...
      for (const warning of runtimeWorkspaceWarnings) {
        await onLog("stderr", `[orchestorai] ${warning}\n`);
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected ORCHESTORAI_API_KEY",
        );
      }
