...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@orchestorai/db";
...
interface IncomingMessageWithContext extends IncomingMessage {
  orchestoraiUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).orchestoraiUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.orchestoraiUpgradeContext = context;
