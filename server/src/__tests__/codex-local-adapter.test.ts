...
import { isCodexUnknownSessionError, parseCodexJsonl } from "@orchestorai/adapter-codex-local/server";
import { parseCodexStdoutLine } from "@orchestorai/adapter-codex-local/ui";
  ...
  it("parses turn and reasoning lifecycle events", () => {
    ...
    expect(
      parseCodexStdoutLine(
        JSON.stringify({
          type: "item.completed",
          item: { id: "item_1", type: "reasoning", text: "**Preparing to use orchestorai skill**" },
        }),
        ts,
      ),
    ).toEqual([
      { kind: "thinking", ts, text: "**Preparing to use orchestorai skill**" },
    ]);
  });
