...
type OrchestorAISprite = (typeof ORCHESTORAI_SPRITES)[number];
...
function spriteSize(sprite: OrchestorAISprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = ORCHESTORAI_SPRITES[(baseCol + baseRow) % ORCHESTORAI_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = ORCHESTORAI_SPRITES[Math.floor(Math.random() * ORCHESTORAI_SPRITES.length)]!;
