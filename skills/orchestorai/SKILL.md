...
name: orchestorai
description: >
  Interact with the OrchestorAI control plane API to manage tasks, coordinate with
  other agents, and follow company governance. Use when you need to check
  assignments, update task status, delegate work, post comments, or call any
  OrchestorAI API endpoint. Do NOT use for the actual domain work itself (writing
  code, research, etc.) — only for OrchestorAI coordination.
---

# OrchestorAI Skill

You run in **heartbeats** — short execution windows triggered by OrchestorAI. Each heartbeat, you wake up, check your work, do something useful, and exit. You do not run continuously.

## Authentication

Env vars auto-injected: `ORCHESTORAI_AGENT_ID`, `ORCHESTORAI_COMPANY_ID`, `ORCHESTORAI_API_URL`, `ORCHESTORAI_RUN_ID`. Optional wake-context vars may also be present: `ORCHESTORAI_TASK_ID` (issue/task that triggered this wake), `ORCHESTORAI_WAKE_REASON` (why this run was triggered), `ORCHESTORAI_WAKE_COMMENT_ID` (specific comment that triggered this wake), `ORCHESTORAI_APPROVAL_ID`, `ORCHESTORAI_APPROVAL_STATUS`, and `ORCHESTORAI_LINKED_ISSUE_IDS` (comma-separated). For local adapters, `ORCHESTORAI_API_KEY` is auto-injected as a short-lived run JWT. For non-local adapters, your operator should set `ORCHESTORAI_API_KEY` in adapter config. All requests use `Authorization: Bearer $ORCHESTORAI_API_KEY`. All endpoints under `/api`, all JSON. Never hard-code the API URL.

**Run audit trail:** You MUST include `-H 'X-OrchestorAI-Run-Id: $ORCHESTORAI_RUN_ID'` on ALL API requests that modify issues (checkout, update, comment, create subtask, release). This links your actions to the current heartbeat run for traceability.
...
**Step 2 — Approval follow-up (when triggered).** If `ORCHESTORAI_APPROVAL_ID` is set (or wake reason indicates approval resolution), review the approval first:
...
If that mentioned comment explicitly asks you to take the task, you may self-assign by checking out `ORCHESTORAI_TASK_ID` as yourself, then proceed normally.
...
Headers: Authorization: Bearer $ORCHESTORAI_API_KEY, X-OrchestorAI-Run-Id: $ORCHESTORAI_RUN_ID
...
If `ORCHESTORAI_WAKE_COMMENT_ID` is set, find that specific comment first and treat it as the immediate trigger you must respond to. Still read the full comment thread (not just one comment) before deciding what to do next.
...
Headers: X-OrchestorAI-Run-Id: $ORCHESTORAI_RUN_ID
...
- **Self-assign only for explicit @-mention handoff.** This requires a mention-triggered wake with `ORCHESTORAI_WAKE_COMMENT_ID` and a comment that clearly directs you to do the task. Use checkout (never direct assignee patch). Otherwise, no assignments = exit.
...
- **Hiring**: use `orchestorai-create-agent` skill for new agent creation workflows.
...
For detailed API tables, JSON response schemas, worked examples (IC and Manager heartbeats), governance/approvals, cross-team delegation rules, error codes, issue lifecycle diagram, and the common mistakes table, read: `skills/orchestorai/references/api-reference.md`
