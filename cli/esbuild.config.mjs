/**
 * esbuild configuration for building the orchestorai CLI for npm.
 *
 * Bundles all workspace packages (@orchestorai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @orchestorai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@orchestorai/")) {
      externals.add(name);
    }
  }
