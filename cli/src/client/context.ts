...
function findContextFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".orchestorai", DEFAULT_CONTEXT_BASENAME);
...
export function resolveContextPath(overridePath?: string): string {
  ...
  if (process.env.ORCHESTORAI_CONTEXT) return path.resolve(process.env.ORCHESTORAI_CONTEXT);
