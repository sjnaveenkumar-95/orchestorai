...
function tempConfigPath(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "orchestorai-jwt-env-"));
...
describe("agent jwt env helpers", () => {
  beforeEach(() => {
    ...
    delete process.env.ORCHESTORAI_AGENT_JWT_SECRET;
  });
  ...
  it("writes .env next to explicit config path", () => {
    ...
    expect(contents).toContain("ORCHESTORAI_AGENT_JWT_SECRET=");
  });

  it("loads secret from .env next to explicit config path", () => {
    const configPath = tempConfigPath();
    const envPath = resolveAgentJwtEnvFile(configPath);
    fs.writeFileSync(envPath, "ORCHESTORAI_AGENT_JWT_SECRET=test-secret\n", { mode: 0o600 });

    const loaded = readAgentJwtSecretFromEnv(configPath);
    expect(loaded).toBe("test-secret");
    expect(process.env.ORCHESTORAI_AGENT_JWT_SECRET).toBe("test-secret");
  });

  it("doctor check passes when secret exists in adjacent .env", () => {
    const configPath = tempConfigPath();
    const envPath = resolveAgentJwtEnvFile(configPath);
    fs.writeFileSync(envPath, "ORCHESTORAI_AGENT_JWT_SECRET=check-secret\n", { mode: 0o600 });
