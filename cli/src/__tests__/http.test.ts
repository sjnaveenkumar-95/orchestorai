...
describe("OrchestorAIApiClient", () => {
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new OrchestorAIApiClient({
    ...
    expect(headers["x-orchestorai-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new OrchestorAIApiClient({ apiBase: "http://localhost:3100" });
