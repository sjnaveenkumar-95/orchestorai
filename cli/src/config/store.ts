...
import { orchestoraiConfigSchema, type OrchestorAIConfig } from "./schema.js";
import {
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".orchestorai", DEFAULT_CONFIG_BASENAME);
...
export function resolveConfigPath(overridePath?: string): string {
  ...
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath(resolveOrchestorAIInstanceId());
}
...
export function readConfig(configPath?: string): OrchestorAIConfig | null {
  const filePath = resolveConfigPath(configPath);
  if (!fs.existsSync(filePath)) return null;
  const raw = parseJson(filePath);
  const migrated = migrateLegacyConfig(raw);
  const parsed = orchestoraiConfigSchema.safeParse(migrated);
  if (!parsed.success) {
