...
const DEFAULT_AGENT_JWT_ISSUER = "orchestorai";
const DEFAULT_AGENT_JWT_AUDIENCE = "orchestorai-api";
...
function defaultSecretsKeyFilePath(): string {
  return resolveDefaultSecretsKeyFilePath(resolveOrchestorAIInstanceId());
}
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveOrchestorAIInstanceId());
}

export async function envCommand(opts: { config?: string }): Promise<void> {
  p.intro(pc.bgCyan(pc.black(" orchestorai env ")));

  const configPath = resolveConfigPath(opts.config);
  let config: OrchestorAIConfig | null = null;
...
function collectDeploymentEnvRows(config: OrchestorAIConfig | null, configPath: string): EnvVarRow[] {
  ...
  const publicUrl =
    process.env.ORCHESTORAI_PUBLIC_URL ??
    process.env.ORCHESTORAI_AUTH_PUBLIC_BASE_URL ??
  ...
  const publicUrlSource: EnvSource =
    process.env.ORCHESTORAI_PUBLIC_URL
      ? "env"
      : process.env.ORCHESTORAI_AUTH_PUBLIC_BASE_URL || process.env.BETTER_AUTH_URL || process.env.BETTER_AUTH_BASE_URL
        ? "env"
        : config?.auth?.publicBaseUrl
          ? "config"
          : "missing";
  ...
  const secretsProvider =
    process.env.ORCHESTORAI_SECRETS_PROVIDER ??
    config?.secrets?.provider ??
    DEFAULT_SECRETS_PROVIDER;
  const secretsStrictMode =
    process.env.ORCHESTORAI_SECRETS_STRICT_MODE ??
    String(config?.secrets?.strictMode ?? false);
  const secretsKeyFilePath =
    process.env.ORCHESTORAI_SECRETS_MASTER_KEY_FILE ??
    config?.secrets?.localEncrypted?.keyFilePath ??
    defaultSecretsKeyFilePath();
  const storageProvider =
    process.env.ORCHESTORAI_STORAGE_PROVIDER ??
    config?.storage?.provider ??
    DEFAULT_STORAGE_PROVIDER;
  const storageLocalDir =
    process.env.ORCHESTORAI_STORAGE_LOCAL_DIR ??
    config?.storage?.localDisk?.baseDir ??
    defaultStorageBaseDir();
  const storageS3Bucket =
    process.env.ORCHESTORAI_STORAGE_S3_BUCKET ??
    config?.storage?.s3?.bucket ??
    "orchestorai";
  const storageS3Region =
    process.env.ORCHESTORAI_STORAGE_S3_REGION ??
    config?.storage?.s3?.region ??
    "us-east-1";
  const storageS3Endpoint =
    process.env.ORCHESTORAI_STORAGE_S3_ENDPOINT ??
    config?.storage?.s3?.endpoint ??
    "";
  const storageS3Prefix =
    process.env.ORCHESTORAI_STORAGE_S3_PREFIX ??
    config?.storage?.s3?.prefix ??
    "";
  const storageS3ForcePathStyle =
    process.env.ORCHESTORAI_STORAGE_S3_FORCE_PATH_STYLE ??
    String(config?.storage?.s3?.forcePathStyle ?? false);

  const rows: EnvVarRow[] = [
    {
      key: "ORCHESTORAI_AGENT_JWT_SECRET",
    ...
    },
    {
      ...
      source: process.env.BETTER_AUTH_TRUSTED_ORIGINS
        ...
        : trustedOriginsDefault
          ...
          : "missing",
      required: false,
      note: "Comma-separated auth origin allowlist (auto-derived from ORCHESTORAI_PUBLIC_URL when possible)",
    },
    {
      key: "ORCHESTORAI_AGENT_JWT_TTL_SECONDS",
      value: process.env.ORCHESTORAI_AGENT_JWT_TTL_SECONDS ?? DEFAULT_AGENT_JWT_TTL_SECONDS,
      source: process.env.ORCHESTORAI_AGENT_JWT_TTL_SECONDS ? "env" : "default",
      required: false,
      note: "JWT lifetime in seconds",
    },
    {
      key: "ORCHESTORAI_AGENT_JWT_ISSUER",
      value: process.env.ORCHESTORAI_AGENT_JWT_ISSUER ?? DEFAULT_AGENT_JWT_ISSUER,
      source: process.env.ORCHESTORAI_AGENT_JWT_ISSUER ? "env" : "default",
      required: false,
      note: "JWT issuer",
    },
    {
      key: "ORCHESTORAI_AGENT_JWT_AUDIENCE",
      value: process.env.ORCHESTORAI_AGENT_JWT_AUDIENCE ?? DEFAULT_AGENT_JWT_AUDIENCE,
      source: process.env.ORCHESTORAI_AGENT_JWT_AUDIENCE ? "env" : "default",
      required: false,
      note: "JWT audience",
    },
    {
    ...
    {
      key: "ORCHESTORAI_SECRETS_PROVIDER",
      value: secretsProvider,
      source: process.env.ORCHESTORAI_SECRETS_PROVIDER
        ? "env"
        : config?.secrets?.provider
          ? "config"
          : "default",
    ...
    {
      key: "ORCHESTORAI_SECRETS_STRICT_MODE",
      value: secretsStrictMode,
      source: process.env.ORCHESTORAI_SECRETS_STRICT_MODE
        ? "env"
    ...
    {
      key: "ORCHESTORAI_SECRETS_MASTER_KEY_FILE",
      value: secretsKeyFilePath,
      source: process.env.ORCHESTORAI_SECRETS_MASTER_KEY_FILE
        ? "env"
        : config?.secrets?.localEncrypted?.keyFilePath
          ? "config"
          : "default",
    ...
    {
      key: "ORCHESTORAI_STORAGE_PROVIDER",
      value: storageProvider,
      source: process.env.ORCHESTORAI_STORAGE_PROVIDER
        ? "env"
        : config?.storage?.provider
          ? "config"
          : "default",
    ...
    {
      key: "ORCHESTORAI_STORAGE_LOCAL_DIR",
      value: storageLocalDir,
      source: process.env.ORCHESTORAI_STORAGE_LOCAL_DIR
        ? "env"
        : config?.storage?.localDisk?.baseDir
          ? "config"
          : "default",
    ...
    {
      key: "ORCHESTORAI_STORAGE_S3_BUCKET",
      value: storageS3Bucket,
      source: process.env.ORCHESTORAI_STORAGE_S3_BUCKET
        ? "env"
        : config?.storage?.s3?.bucket
          ? "config"
          : "default",
    ...
    {
      key: "ORCHESTORAI_STORAGE_S3_REGION",
      value: storageS3Region,
      source: process.env.ORCHESTORAI_STORAGE_S3_REGION
        ? "env"
        : config?.storage?.s3?.region
          ? "config"
          : "default",
    ...
    {
      key: "ORCHESTORAI_STORAGE_S3_ENDPOINT",
      value: storageS3Endpoint,
      source: process.env.ORCHESTORAI_STORAGE_S3_ENDPOINT
        ? "env"
        : config?.storage?.s3?.endpoint
          ? "config"
          : "default",
    ...
    {
      key: "ORCHESTORAI_STORAGE_S3_PREFIX",
      value: storageS3Prefix,
      source: process.env.ORCHESTORAI_STORAGE_S3_PREFIX
        ? "env"
        : config?.storage?.s3?.prefix
          ? "config"
          : "default",
    ...
    {
      key: "ORCHESTORAI_STORAGE_S3_FORCE_PATH_STYLE",
      value: storageS3ForcePathStyle,
      source: process.env.ORCHESTORAI_STORAGE_S3_FORCE_PATH_STYLE
        ? "env"
  ...
  if (process.env.ORCHESTORAI_CONFIG || configPath !== defaultConfigPath) {
    rows.push({
      key: "ORCHESTORAI_CONFIG",
      value: process.env.ORCHESTORAI_CONFIG ?? configPath,
      source: process.env.ORCHESTORAI_CONFIG ? "env" : "default",
      required: false,
