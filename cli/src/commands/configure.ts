...
function defaultConfig(): OrchestorAIConfig {
...
export async function configure(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" orchestorai configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `orchestorai onboard` first.");
    p.outro("");
    return;
  }

  let config: OrchestorAIConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because ORCHESTORAI_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
