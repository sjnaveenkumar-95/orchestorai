...
import { createDb, instanceUserRoles, invites } from "@orchestorai/db";
...
export async function bootstrapCeoInvite(opts: {
  ...
  const config = readConfig(configPath);
  if (!config) {
    p.log.error(`No config found at ${configPath}. Run ${pc.cyan("orchestorai onboard")} first.`);
    return;
  }
  ...
  try {
    ...
    p.log.info("If using embedded-postgres, start the OrchestorAI server and run this command again.");
  }
}
