...
import { ApiRequestError, OrchestorAIApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to OrchestorAI config file")
    .option("-d, --data-dir <path>", "OrchestorAI data directory root (isolates state from ~/.orchestorai)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the OrchestorAI API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.ORCHESTORAI_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const apiKey =
    options.apiKey?.trim() ||
    process.env.ORCHESTORAI_API_KEY?.trim() ||
    readKeyFromProfileEnv(profile);

  const companyId =
    options.companyId?.trim() ||
    process.env.ORCHESTORAI_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set ORCHESTORAI_COMPANY_ID, or set context profile companyId via `orchestorai context set`.",
    );
  }

  const api = new OrchestorAIApiClient({ apiBase, apiKey });
  return {
    api,
    companyId,
    profileName,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.ORCHESTORAI_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.ORCHESTORAI_SERVER_PORT || "");
