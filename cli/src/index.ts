...
const DATA_DIR_OPTION_HELP =
  "OrchestorAI data directory root (isolates state from ~/.orchestorai)";

program
  .name("orchestorai")
  .description("OrchestorAI CLI — setup, diagnose, and configure your instance")
  .version("0.2.7");
...
program
  ...
  .option("--run", "Start OrchestorAI immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your OrchestorAI setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "orchestorai")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run OrchestorAI")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
  .option("-i, --instance <id>", "Local instance id (default: default)")
  .option("-y, --yes", "Use quickstart defaults when onboarding is needed in non-interactive environments", false)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the OrchestorAI server API")
