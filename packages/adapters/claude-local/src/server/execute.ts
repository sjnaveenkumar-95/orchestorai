...
import type { RunProcessResult } from "@orchestorai/adapter-utils/server-utils";
import {
  asString,
  asNumber,
  asBoolean,
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stderr",
        `[orchestorai] Claude resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );
