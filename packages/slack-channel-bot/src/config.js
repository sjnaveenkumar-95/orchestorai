...
import { translateLegacyCodexToModel } from "../../slack-agent-core/src/config.js";

const PACKAGE_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");

export function findNearestDotenvPath(cwd = PACKAGE_ROOT, processEnv = process.env) {
  const explicitEnvPath = String(
    processEnv.SLACK_CHANNEL_BOT_ENV_FILE ||
      processEnv.SLACK_BOT_ENV_FILE ||
      processEnv.SLACK_AGENT_ENV_FILE ||
      "",
  ).trim();
  if (explicitEnvPath) {
...
export function loadDotenvForCwd(cwd = PACKAGE_ROOT, processEnv = process.env) {
...
loadOrchestorAIEnvIntoProcess({
  ...
  keys: [
    ...
    "ORCHESTORAI_TASK_PREFIX",
    "ORCHESTORAI_TRIGGER_MENTIONS",
    "ORCHESTORAI_AGENT_MAPPINGS_JSON",
    "ORCHESTORAI_PROJECT_MAPPINGS_JSON",
    "ORCHESTORAI_SYNC_REPLIES",
    "ORCHESTORAI_NOTIFICATION_MODE",
  ],
});
...
function applyEnvOverrides(config, cwd) {
  ...
  codex.reasoningEffort = normalizePolicy(
    env.CODEX_REASONING_EFFORT || env.CODEX_THINKING || codex.reasoningEffort,
  ...
  codex.systemPrompt = (env.CODEX_SYSTEM_PROMPT_CHANNEL || env.CODEX_SYSTEM_PROMPT || codex.systemPrompt || "").trim();
  codex.directMessageSystemPrompt = (
    env.CODEX_SYSTEM_PROMPT_DM || codex.directMessageSystemPrompt || ""
  ).trim();
...
function normalizeConfig(config, cwd) {
  ...
  normalized.codex.reasoningEffort = normalizePolicy(
    normalized.codex.reasoningEffort,
    ["minimal", "low", "medium", "high"],
    "",
  );
  normalized.codex.systemPrompt = String(normalized.codex.systemPrompt || "").trim();
  normalized.codex.directMessageSystemPrompt = String(normalized.codex.directMessageSystemPrompt || "").trim();
  ...
  normalized.model = translateLegacyCodexToModel(normalized.codex);
...
export function loadConfig(cwd = process.cwd(), options = {}) {
  ...
  if (options.allowDirectMessages !== true) {
    normalized.slack.dm.enabled = false;
  }
