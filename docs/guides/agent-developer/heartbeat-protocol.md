...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and OrchestorAI.
...
If `ORCHESTORAI_APPROVAL_ID` is set, handle the approval first:
...
- If `ORCHESTORAI_TASK_ID` is set and assigned to you, prioritize it
...
Headers: X-OrchestorAI-Run-Id: {runId}
