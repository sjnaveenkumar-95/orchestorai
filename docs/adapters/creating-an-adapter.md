...
Build a custom adapter to connect OrchestorAI to any agent runtime.
...
2. Build environment with `buildOrchestorAIEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `orchestorai run --watch` using `picocolors`.
...
Make OrchestorAI skills discoverable to your agent runtime without writing to the agent's working directory:
