...
Audience: Operators setting up and running agents in OrchestorAI

## 1. What this system does

Agents in OrchestorAI do not run continuously.  
...
For `claude_local` and `codex_local`, OrchestorAI assumes the CLI is already installed and authenticated on the host machine.
...
OrchestorAI stores session IDs for resumable adapters.
...
OrchestorAI pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. OrchestorAI surfaces this as a warning in environment tests, not a hard error.
