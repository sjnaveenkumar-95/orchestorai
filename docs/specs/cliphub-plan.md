# ClipHub: Marketplace for OrchestorAI Team Configurations

> The "app store" for whole-company AI teams — pre-built OrchestorAI configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for OrchestorAI-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | OrchestorAI company (agents, projects, governance) |
...
A complete OrchestorAI company configuration:
...
Single-agent configurations designed to plug into a OrchestorAI org:
...
Portable skill files that any OrchestorAI agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with OrchestorAI's skill loading system
...
interface Listing {
  ...
  orchestoraiVersionMin: string;     // Minimum OrchestorAI version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target OrchestorAI company
...
| `POST` | `/api/listings/:id/install` | Install to OrchestorAI company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target OrchestorAI company (or create new)
  → ClipHub API calls OrchestorAI API to:
  ...
  → Redirect to OrchestorAI dashboard with new team running
```
...
### 5.3 Creator: Export from OrchestorAI → Publish

```
Running OrchestorAI company → "Export as Blueprint" (CLI or UI)
  → OrchestorAI exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (OrchestorAI brand blue/purple)
...
| Install flow | One-click deploy to OrchestorAI company |
...
{
  "targetCompanyId": "uuid",         // Existing OrchestorAI company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `orchestorai-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as OrchestorAI
- **Backend**: Node.js API (or extend OrchestorAI server)
- **Database**: Postgres (can share OrchestorAI's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with OrchestorAI auth (or OAuth2)

### 9.2 Integration with OrchestorAI

ClipHub can be:
- **Option A**: A separate app that calls OrchestorAI's API to install blueprints
- **Option B**: A built-in section of the OrchestorAI UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing OrchestorAI UI and API.
...
- [ ] Install flow: blueprint → OrchestorAI company
...
- [ ] "Export from OrchestorAI" CLI command
