...
OrchestorAI supports two runtime modes with different security profiles.
...
```sh
pnpm orchestorai allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, OrchestorAI emits a one-time claim URL at startup:
...
```sh
ORCHESTORAI_DEPLOYMENT_MODE=authenticated pnpm orchestorai run
```
