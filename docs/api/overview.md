...
OrchestorAI exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`ORCHESTORAI_API_KEY`)
...
- Run audit trail: include `X-OrchestorAI-Run-Id` header on all mutating requests during heartbeats
