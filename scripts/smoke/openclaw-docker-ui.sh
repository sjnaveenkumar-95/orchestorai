...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-$OPENCLAW_TMP_DIR/openclaw-orchestorai-smoke}"
...
ORCHESTORAI_HOST_PORT="${ORCHESTORAI_HOST_PORT:-3100}"
ORCHESTORAI_HOST_FROM_CONTAINER="${ORCHESTORAI_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.orchestorai-openclaw.override.yml"
...
detect_orchestorai_base_url() {
  ...
  for candidate in "$ORCHESTORAI_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${ORCHESTORAI_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${ORCHESTORAI_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
orchestorai_base_url="$(detect_orchestorai_base_url || true)"
...
OrchestorAI URL for OpenClaw container:
EOF
  if [[ -n "$orchestorai_base_url" ]]; then
    cat <<EOF
  $orchestorai_base_url
  (Use this base URL for invite/onboarding links from inside OpenClaw Docker.)
EOF
  else
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${ORCHESTORAI_HOST_PORT}
  (Do not use http://127.0.0.1:${ORCHESTORAI_HOST_PORT} inside the container.)
  If OrchestorAI rejects the host, run on host machine:
    pnpm orchestorai allowed-hostname host.docker.internal
  Then restart OrchestorAI and re-run this script.
EOF
  fi
  cat <<EOF
