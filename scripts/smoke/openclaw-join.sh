...
API_BASE="${ORCHESTORAI_API_URL%/}/api"
COMPANY_ID="${COMPANY_ID:-${ORCHESTORAI_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-orchestorai-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-orchestorai-openclaw-smoke}"
SMOKE_PORT="${SMOKE_PORT:-19091}"
SMOKE_TIMEOUT_SEC="${SMOKE_TIMEOUT_SEC:-45}"

AUTH_HEADERS=()
if [[ -n "${ORCHESTORAI_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=(-H "Authorization: ${ORCHESTORAI_AUTH_HEADER}")
fi
if [[ -n "${ORCHESTORAI_COOKIE:-}" ]]; then
  AUTH_HEADERS+=(-H "Cookie: ${ORCHESTORAI_COOKIE}")
fi
...
Provide one of:
  ORCHESTORAI_AUTH_HEADER=\"Bearer <board-token>\"
  ORCHESTORAI_COOKIE=\"<board-session-cookie>\"
...
log "checking OrchestorAI health"
...
assert_status "200"
if ! grep -q "OrchestorAI OpenClaw Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.orchestorai.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
