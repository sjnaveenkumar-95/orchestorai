...
- local mode that still feels instant (`npx orchestorai run` and go)
...
2. CLI command `pnpm orchestorai auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm orchestorai onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm orchestorai onboard`).
...
- expose key instance settings in API + CLI (`orchestorai configure` / `orchestorai onboard`)
...
7. `pnpm orchestorai onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
