...
OrchestorAI uses PostgreSQL via [Drizzle ORM](https://orm.drizzle.team/). There are three ways to run the database, from simplest to most production-ready.
...
1. Creates a `~/.orchestorai/instances/default/db/` directory for storage
2. Ensures the `orchestorai` database exists
3. Runs migrations automatically for empty databases
4. Starts serving requests

Data persists across restarts in `~/.orchestorai/instances/default/db/`. To reset local dev data, delete that directory.

This mode is ideal for local development and one-command installs.

Docker note: the Docker quickstart image also uses embedded PostgreSQL by default. Persist `/orchestorai` to keep DB state across container restarts (see `doc/DOCKER.md`).
...
| `DATABASE_URL` | Mode |
|---|---|
| Not set | Embedded PostgreSQL (`~/.orchestorai/instances/default/db/`) |
...
OrchestorAI stores secret metadata and versions in:
...
- Default key file: `~/.orchestorai/instances/default/secrets/master.key` (auto-created if missing).
- CLI config location: `~/.orchestorai/instances/default/config.json` under `secrets.localEncrypted.keyFilePath`.

Optional overrides:

- `ORCHESTORAI_SECRETS_MASTER_KEY` (32-byte key as base64, hex, or raw 32-char string)
- `ORCHESTORAI_SECRETS_MASTER_KEY_FILE` (custom key file path)
