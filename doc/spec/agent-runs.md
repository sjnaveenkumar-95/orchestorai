...
This spec defines how OrchestorAI actually runs agents while staying runtime-agnostic.
...
1. OrchestorAI is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, OrchestorAI manages lifecycle, and on exit OrchestorAI parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `orchestorai.skill` (shared OrchestorAI skill text block)
...
2. Preferred transport is env vars (`ORCHESTORAI_*`) injected at runtime.
