...
This document covers how to build and publish the `orchestorai` CLI package to npm.

## Prerequisites

- Node.js 20+
- pnpm 9.15+
- An npm account with publish access to the `orchestorai` package
...
3. **esbuild bundle** — bundles the CLI entry point (`cli/src/index.ts`) and all workspace package code (`@orchestorai/*`) into a single file at `cli/dist/index.js`. External npm dependencies (express, postgres, etc.) are kept as regular imports.
...
{
  "dependencies": {
    "@orchestorai/server": "workspace:*",
    "@orchestorai/db": "workspace:*"
  }
}
```
...
The CLI is a monorepo package that imports code from `@orchestorai/server`, `@orchestorai/db`, `@orchestorai/shared`, and several adapter packages. These workspace packages don't exist on npm.

**esbuild** bundles all workspace TypeScript code into a single `dist/index.js` file (~250kb). External npm packages (express, postgres, zod, etc.) are left as normal `import` statements — they get installed by npm when a user runs `npx orchestorai onboard`.
