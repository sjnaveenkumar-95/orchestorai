# OrchestorAI Module System

## Overview

OrchestorAI's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends OrchestorAI's API, UI, and data model | Yes |
...
modules/
  observability/
    orchestorai.module.json     # manifest (required)
    src/
...
### Manifest (`orchestorai.module.json`)
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@orchestorai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@orchestorai/core";
...
```
1. Scan modules/ directory for orchestorai.module.json manifests
...
```jsonc
// orchestorai.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@orchestorai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@orchestorai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:orchestorai/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.orchestorai.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm orchestorai store list                    # browse available modules and templates
pnpm orchestorai store install <module-id>     # install a module
pnpm orchestorai store import <template-id>    # import a company template
pnpm orchestorai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one OrchestorAI instance | `server:started` |
...
4. **Module config** — `orchestorai.config.json` with per-module config, env var interpolation
...
10. **`@orchestorai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
