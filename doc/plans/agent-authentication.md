...
Agents need API keys to authenticate with OrchestorAI. The current approach
...
**Trust model:** The adapter process runs on the same machine as the OrchestorAI
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** OrchestorAI generates a token and passes it directly to the agent
...
`ORCHESTORAI_API_URL`, `ORCHESTORAI_AGENT_ID`, `ORCHESTORAI_COMPANY_ID`. We need to
add a `ORCHESTORAI_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a OrchestorAI CLI
...
```
orchestorai auth login
# Opens browser -> user confirms -> token stored at ~/.orchestorai/credentials
```
...
managed by the OrchestorAI server itself.
...
1. A company admin (user or agent) generates an **invite URL** from OrchestorAI.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The OrchestorAI SKILL.md (or a link to it)
   - What information OrchestorAI needs from the agent (e.g. webhook URL, adapter
...
5. OrchestorAI stores the pending registration.
...
7. On approval, OrchestorAI provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** OrchestorAI issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.orchestorai.ing/skills/orchestorai/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How OrchestorAI should send you heartbeats",
...
1. Generate a OrchestorAI invite link for the company.
...
4. A OrchestorAI company member approves the new agent.
5. OrchestorAI begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `ORCHESTORAI_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `orchestorai auth login` for developer-managed remote agents.                                      |
