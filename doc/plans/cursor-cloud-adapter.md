...
This document defines the V1 design for a OrchestorAI adapter that integrates with
...
4. synthesized stdout events for OrchestorAI UI/CLI
...
3. **Agent callback auth to OrchestorAI** uses a bootstrap exchange flow (no long-lived OrchestorAI key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from OrchestorAI endpoints, not full SKILL.md prompt injection.
...
- `orchestoraiPublicUrl` (optional override; else `ORCHESTORAI_PUBLIC_URL` env)
...
## OrchestorAI Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `ORCHESTORAI_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.orchestoraiPublicUrl`
2. `process.env.ORCHESTORAI_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived OrchestorAI credentials in prompt text.

Flow:

1. Before launch/follow-up, OrchestorAI mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
   - short TTL (for example 10 minutes)
2. Adapter includes only:
   - `orchestoraiPublicUrl`
   - exchange endpoint path
   - bootstrap token
3. Cursor agent calls:
   - `POST /api/agent-auth/exchange`
4. OrchestorAI validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all OrchestorAI API calls.
...
1. Prompt includes a compact instruction to fetch skills from OrchestorAI.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/orchestorai`
   - `GET /api/skills/orchestorai-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <orchestoraiPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active OrchestorAI run context.
...
5. `orchestoraiPublicUrl` present and reachable shape-valid
...
- poll interval
- timeout/grace
- orchestorai public URL override
- enable webhooks
...
| Skill delivery | local skill dir injection | authenticated fetch from OrchestorAI skill endpoints |
| OrchestorAI auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from OrchestorAI context.
3. Add richer PR metadata surfacing in OrchestorAI UI.
