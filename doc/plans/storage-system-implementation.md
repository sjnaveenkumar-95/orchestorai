...
Add a single storage subsystem for OrchestorAI that supports:
...
- API serves attachment bytes through authenticated OrchestorAI endpoints.
...
- Default local path is under instance root: `~/.orchestorai/instances/<instanceId>/data/storage`.
...
- `orchestorai onboard` writes a valid `storage` config block by default.
- `orchestorai configure --section storage` can switch between local and s3 modes.
...
- `orchestorai doctor` reports actionable storage status.
