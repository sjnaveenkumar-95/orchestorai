...
ClipHub is the public registry where people share, discover, and download OrchestorAI company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to OrchestorAI what a package registry is to a programming language. OrchestorAI already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in OrchestorAI — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own OrchestorAI instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the OrchestorAI spec. It contains:
...
**Install (fresh start):**
```
orchestorai install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local OrchestorAI instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within OrchestorAI, export your company as a template, then publish:

```
orchestorai export --template my-company
orchestorai publish cliphub my-company
```
...
ClipHub is a **separate service** from OrchestorAI itself. OrchestorAI is self-hosted; ClipHub is a hosted registry that OrchestorAI instances talk to.
...
| **OrchestorAI CLI** | `orchestorai install`, `orchestorai publish`, `orchestorai cliphub sync` — built into OrchestorAI |
| **OrchestorAI UI** | "Browse ClipHub" panel in the OrchestorAI web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with OrchestorAI) |
| Backend | TypeScript + Hono (consistent with OrchestorAI) |
| Database | PostgreSQL |
...
4. Run `orchestorai install cliphub:acme/lean-saas-shop`
5. OrchestorAI creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in OrchestorAI until it works well
2. Export: `orchestorai export --template my-agency`
3. Publish: `orchestorai publish cliphub my-agency`
...
3. Install just that agent: `orchestorai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to OrchestorAI

ClipHub is **not required** to use OrchestorAI. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to OrchestorAI what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `orchestorai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in OrchestorAI web UI
- [ ] `orchestorai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with OrchestorAI's own philosophy)
